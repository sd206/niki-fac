...
/** Resolves the active family (first membership for now). */
...
export function useUpdateEvent() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, patch }: { id: string; patch: UpdateEventRequest }) => {
      const res = await apiFetch<CalendarEvent>(`/api/v1/calendar/events/${id}`, {
        method: "PATCH",
...
export function useRevokeInvitation() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ familyId, invitationId }: { familyId: string; invitationId: string }) => {
      await apiFetch(
...
export function useDeleteBudget() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      await apiFetch(`/api/v1/finance/budgets/${id}`, { method: "DELETE" });
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["budgets"] }),
  });
}
...
export function useDeleteExpense() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      await apiFetch(`/api/v1/finance/expenses/${id}`, { method: "DELETE" });
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["expenses", "budgets"] }),
  });
}
...
export function useUpdateSavingsGoal() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, patch }: { id: string; patch: UpdateSavingsGoalRequest }) => {
      const res = await apiFetch<SavingsGoal>(`/api/v1/finance/savings-goals/${id}`, {
        method: "PATCH",
        body: JSON.stringify(patch),
      });
      if (res.error) throw new Error(res.error.message);
...
export function useDeleteSavingsGoal() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      await apiFetch(`/api/v1/finance/savings-goals/${id}`, { method: "DELETE" });
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["savingsGoals"] }),
  });
}
...
export function useRecipes(familyId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await apiFetch<Recipe[]>(`/api/v1/meals/recipes?familyId=${familyId}`);
      if (res.error) throw new Error(res.error.message);
      return res.data ?? [];
    },
  });
...
export function useCreateRecipe() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (input: CreateRecipeRequest) => {
      const res = await apiFetch<Recipe>("/api/v1/meals/recipes", {
        method: "POST",
        body: JSON.stringify(input),
      });
      if (res.error) throw new Error(res.error.message);
...
export function useDeleteMeal() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      await apiFetch(`/api/v1/meals/meals/${id}`, { method: "DELETE" });
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["meals"] }),
  });
}
...
export function useUpdateShoppingItem() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, patch }: { id: string; patch: UpdateShoppingItemRequest }) => {
      const res = await apiFetch<ShoppingListItem>(`/api/v1/meals/shopping-list/${id}`, {
        method: "PATCH",
        body: JSON.stringify(patch),
      });
      if (res.error) throw new Error(res.error.message);
...
export function useDeleteTrip() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      await apiFetch(`/api/v1/travel/trips/${id}`, { method: "DELETE" });
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["trips", "reservations", "packing"] }),
  });
}
...
export function useDeleteReservation() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      await apiFetch(`/api/v1/travel/reservations/${id}`, { method: "DELETE" });
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["reservations"] }),
  });
}
...
export function useUpdatePackingItem() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, patch }: { id: string; patch: UpdatePackingItemRequest }) => {
      const res = await apiFetch<PackingItem>(`/api/v1/travel/packing/${id}`, {
        method: "PATCH",
        body: JSON.stringify(patch),
      });
      if (res.error) throw new Error(res.error.message);
...
export function useDeletePackingItem() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      await apiFetch(`/api/v1/travel/packing/${id}`, { method: "DELETE" });
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["packing"] }),
  });
}
...
export function useConversations(familyId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await apiFetch<AIConversation[]>(`/api/v1/ai/conversations?familyId=${familyId}`);
      if (res.error) throw new Error(res.error.message);
      return res.data ?? [];
    },
  });
...
export function useFamilyMetrics(familyId: string | undefined) {
  return useQuery({
    enabled: Boolean(familyId),
    queryKey: ["familyMetrics", familyId],
    queryFn: async () => {
      const res = await apiFetch<FamilyMetrics>(`/api/v1/analytics/family/${familyId}`);
      if (res.error) throw new Error(res.error.message);
      return res.data!;
    },
  });
